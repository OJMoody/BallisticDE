...
//=============================================================================
class RS8X3PrimaryFire extends BallisticRangeAttenFire;
...
function FlashMuzzleFlash()
{
	...
	if (!Instigator.IsFirstPerson() || PlayerController(Instigator.Controller).ViewTarget != Instigator)
		return;
    if (!RS8X3Pistol(Weapon).bSilenced && MuzzleFlash != None)
        MuzzleFlash.Trigger(Weapon, Instigator);
    else if (RS8X3Pistol(Weapon).bSilenced && SMuzzleFlash != None)
...
{
	if (RS8X3Pistol(Weapon).bSilenced)
...
{
	BallisticAttachment(Weapon.ThirdPersonActor).BallisticUpdateHit(Other, HitLocation, HitNormal, Surf, RS8X3Pistol(Weapon).bSilenced, WaterHitLoc);
}

function ServerPlayFiring()
{
	if (RS8X3Pistol(Weapon) != None && RS8X3Pistol(Weapon).bSilenced && SilencedFireSound.Sound != None)
...
function PlayFiring()
{
	...
	if (RS8X3Pistol(Weapon).bSilenced)
		Weapon.SetBoneScale (0, 1.0, RS8X3Pistol(Weapon).SilencerBone);
	else
		Weapon.SetBoneScale (0, 0.0, RS8X3Pistol(Weapon).SilencerBone);
