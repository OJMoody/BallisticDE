...
//=============================================================================
class ProtoSecondaryFire extends BallisticProShotgunFire;
...
simulated function bool AllowFire()
{
	...
	if (ProtoSMG(Weapon).SGShells < 1)
	{
		...
		if (bDryUncock)
			ProtoSMG(BW).bAltNeedCock=true;
		BW.bNeedReload = BW.MayNeedReload(ThisModeNum, 0);

		BW.EmptyFire(ThisModeNum);
		return false;		// Is there ammo in weapon's mag
	}
	else if (ProtoSMG(BW).bAltNeedCock)
		return false;
    return true;
}
...
simulated event ModeDoFire()
{
	...
	Super.ModeDoFire();
    ProtoSMG(Weapon).SGShells--;
	if (Weapon.Role == ROLE_Authority && ProtoSMG(Weapon).SGShells == 0)
		ProtoSMG(BW).bAltNeedCock = true;
}
