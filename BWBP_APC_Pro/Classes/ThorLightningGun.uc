...
simulated function HVCMk9_TrackingZapClassic GetTargetZap()
{
	if (ThirdPersonActor == None)
		return None;
	return ThorAttachment(ThirdPersonActor).TargetZap;
}
...
simulated function SetTargetZap(array<actor> Ts, array<vector> Vs)
{
	...
	if (Role == ROLE_Authority)
		ThorAttachment(ThirdPersonActor).SetTargetZap(Ts, Vs);
...
{
	if (Role == ROLE_Authority)
		ThorAttachment(ThirdPersonActor).SetFreeZap();
	if (!Instigator.IsLocallyControlled())
		return;
	if (FreeZap != None)
...
{
	if (Role == ROLE_Authority)
		ThorAttachment(ThirdPersonActor).KillZap();
	if (Instigator.IsLocallyControlled())
...
defaultproperties
{
	...
	UsedAmbientSound=Sound'BWBP_CC_Sounds.ThorLG.Idle'
	...
	BringUpSound=(Sound=Sound'BWBP_CC_Sounds.ThorLG.Pullout',Volume=0.750000)
	PutDownSound=(Sound=Sound'BWBP_CC_Sounds.ThorLG.Putaway',Volume=0.600000)
	...
	AttachmentClass=Class'BWBP_APC_Pro.ThorAttachment'
