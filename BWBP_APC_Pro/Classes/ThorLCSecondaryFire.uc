class ThorLCSecondaryFire extends BallisticInstantFire;
...
function DoFireEffect()
{
	...
	{
		ThorLCAttachment(Weapon.ThirdPersonActor).bUseAlt=True;	
	...
	if (Target != None)
	{
		...
		if (bDidHeal)
		{
			ThorLightningCannon(BW).BoostTarget = Pawn(Other);
			bShouldUseAltColor=true;
		}
		else
		{
			ThorLightningCannon(BW).DrainTarget = Pawn(Other);
...
function StopFiring()
{
	...
	ThorLightningCannon(BW).BoostTarget = None;
	StopLightning();
