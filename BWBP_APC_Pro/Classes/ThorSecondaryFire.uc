...
//=============================================================================
class ThorSecondaryFire extends BallisticInstantFire;
...
{
	if (ThorLightningGun(Weapon).Spiral == None)
		class'bUtil'.static.InitMuzzleFlash(ThorLightningGun(Weapon).Spiral, class'HVCMk9_RedSpiral', Weapon.DrawScale, Weapon, 'drum');
...
function PlayFiring()
{
	ThorLightningGun(Weapon).ResetArcs();
...
simulated event ModeDoFire()
{
	...
	ThorLightningGun(Weapon).AddHeat(1.2);
	if (Instigator == None)
		return;
...
simulated function bool AllowFire()
{
	if (ChargePower > 0)
		return true;
	//if ((HoldTime <= 0 && ThorLightningGun(Weapon).HeatLevel >= 10) || ThorLightningGun(Weapon).bIsVenting || !super.AllowFire())
		//return false;
	return true;
}
...
simulated event ModeTick(float DT)
{
	...
	//if (Instigator.PhysicsVolume.bWaterVolume)
		//ThorLightningGun(Weapon).AddHeat(DT*3);
	//else
		//ThorLightningGun(Weapon).AddHeat(DT*0.75);
...
{
	ThorAttachment(Weapon.ThirdPersonActor).ChargePower = 255*ChargePower;
