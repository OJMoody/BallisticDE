class FM13FireControl extends RX22AFireControl;
...
simulated function DoFlameHit(FlameHit Hit)
{
	...
	BurnRadius(4, 256, class'DTFM13Burned', 0, Hit.HitLoc, Hit.Instigator);
...
{
	GasNodes[GasNodes.length] = Spawn(class'FM13SurfaceFire',InstigatedBy,,Loc, rotator(Norm));
...
{
	GasNodes[GasNodes.length] = Spawn(class'FM13GasPatch',InstigatedBy,,Loc, rotator(Norm));
...
{
	GasNodes[GasNodes.length] = Spawn(class'FM13GasCloud',InstigatedBy,,Loc);
	RX22AGasCloud(GasNodes[GasNodes.length-1]).GasControl = self;
}
function MakeNewSoaker (Actor Other, Pawn InstigatedBy)
{
	GasNodes[GasNodes.length] = Spawn(class'FM13GasSoak',InstigatedBy,,Other.Location, Other.Rotation);
...
function MakeNewBurner (Actor Other, int Fuel, Pawn InstigatedBy)
{
	...
	PF = Spawn(class'FM13ActorFire',InstigatedBy,,Other.Location, Other.Rotation);
	PF.SetFuel(Fuel);
	PF.Initialize(Other);
