...
//=============================================================================
class ThorPrimaryFire extends BallisticProInstantFire;
...
simulated state DirectFire
{
	...
	{
		if (ThorLightningGun(Weapon).HeatLevel >= 10 || ThorLightningGun(Weapon).bIsVenting || !StreamAllowFire())
		{
			if (Instigator.Role == ROLE_Authority && ThorAttachment(Weapon.ThirdPersonActor).bStreamOn)
				ThorAttachment(Weapon.ThirdPersonActor).EndStream();
			return false;
		}
	...
	function DoFireEffect()
	{
		...
		if (ThorAttachment(Weapon.ThirdPersonActor).StreamEffect == None)
			ThorAttachment(Weapon.ThirdPersonActor).StartStream();
	...
	/*function PlayFiring()
	{
			...
			ThorLightningGun(BW).AddHeat(0.3);
			
			if (Level.NetMode != NM_Client && !bDoOverCharge && ThorLightningGun(Weapon).HeatLevel > 10)
			{
				bDoOverCharge = true;
				Weapon.PlaySound(ThorLightningGun(Weapon).OverHeatSound,Slot_None,0.7,,64,1.0,true);
				ThorLightningGun(Weapon).ClientOverCharge();
	...
	function ServerPlayFiring()
	{
			...
			if (Weapon.Role == Role_Authority && !bDoOverCharge && ThorLightningGun(Weapon).HeatLevel > 10)
			{
				bDoOverCharge = true;
				Weapon.PlaySound(ThorLightningGun(Weapon).OverHeatSound,Slot_None,0.7,,64,1.0,true);
				ThorLightningGun(Weapon).ClientOverCharge();
	...
	simulated event ModeDoFire()
	{
		...
		{
			DoFireEffect();
			ThorLightningGun(Weapon).AddHeat(0.15);
...
simulated state BranchingFire
{
	...
	function DoFireEffect()
	{
		...
		if (Instigator.PhysicsVolume.bWaterVolume)
		{
			...
			ThorLightningGun(Weapon).AddHeat(2.0);
			if (Weapon.AmmoAmount(0) > 5)
				ThorAttachment(Weapon.ThirdPersonActor).DoWaterDischarge();
			ConsumedLoad = 100;
	...
	//				Lure += OldLures[i].Zaps * (VSize(OldLures[i].Loc - Instigator.Location) / 900);
		...
		if (Targets.length > 0 || Lures.length > 0)
			ThorLightningGun(Weapon).SetTargetZap(Targets, Vecs);
		else
			ThorLightningGun(Weapon).SetFreeZap();

	}

	/*simulated function bool AllowFire()
	{
		if (ThorLightningGun(Weapon).HeatLevel >= 10 || ThorLightningGun(Weapon).bIsVenting || !super.AllowFire())
		{
			if (bIsZapping)
			{
				Weapon.PlayOwnedSound(ThorLightningGun(Weapon).OverHeatSound,Slot_None,0.7,,64,1.0,true);
				PlayFireEnd();
			}
			return false;
		}
	...
	function PlayFiring()
	{
		...
		if (FireCount < 1)
		{
			if (level.DetailMode > DM_Low)
				ThorLightningGun(Weapon).InitArcs();
			if (Weapon.HasAnim(FireLoopAnim))
				BW.SafeLoopAnim(FireLoopAnim, FireLoopAnimRate, 0.0, ,"FIRE");
			if (BallisticFireSound.Sound != None)
				Weapon.PlayOwnedSound(BallisticFireSound.Sound,BallisticFireSound.Slot,BallisticFireSound.Volume,BallisticFireSound.bNoOverride,BallisticFireSound.Radius,BallisticFireSound.Pitch,BallisticFireSound.bAtten);
	...
	{
		ThorLightningGun(Weapon).KillZap();
		bIsZapping=false;
		if (MuzzleFlash != None)
