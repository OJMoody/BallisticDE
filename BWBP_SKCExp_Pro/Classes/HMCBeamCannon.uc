	...
	if ( !bTeamSet && (Instigator.PlayerReplicationInfo != None) && (Instigator.PlayerReplicationInfo.Team != None) )
	{
		...
		if ( Instigator.PlayerReplicationInfo.Team.TeamIndex == 0 )
		{
			...
			BFireMode[0].BallisticFireSound.Sound=HMCPrimaryFire(BFireMode[0]).RTeamFireSound;
     		BFireMode[0].MuzzleFlashClass=Class'BWBP_SKCExp_Pro.A48FlashEmitter';
		}
	}
...
defaultproperties
{
     ...
     MagAmmo=500
     FireModeClass(0)=Class'BWBP_SKCExp_Pro.HMCPrimaryFire'
     FireModeClass(1)=Class'BWBP_SKCExp_Pro.HMCSecondaryFire'
     ...
     InventoryGroup=5
     GroupOffset=4
     PickupClass=Class'BWBP_SKCExp_Pro.HMCPickup'
     PlayerViewOffset=(X=6.000000,Y=6.000000,Z=-8.000000)
     BobDamping=1.600000
     AttachmentClass=Class'BWBP_SKCExp_Pro.HMCAttachment'
