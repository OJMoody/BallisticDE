...
//=============================================================================
class AK91_ChargeControl extends Info;
...
{
	local AK91_ZapProjectile Proj1;
	local AK91_ZapProjectile Proj2;
	local int i;

	if (triggerWeaponType == 1)
	{
		Proj1 = Spawn (class'AK91_ZapProjectile',InstigatedBy,, Start, Rotator(End-Start));
		if (Proj1 != None)
		{
...
{
	local AK91_ActorFire PF1;
	local AK91_ActorFire PF2;

	if (triggerType == 1)
	{
		PF1 = Spawn(class'AK91_ActorFire',InstigatedBy,,Other.Location, Other.Rotation);
		PF1.SetFuel(5);
...
function MakeNewExploder (Actor Other, Pawn InstigatedBy, int triggerType)
{
	...
	local AK91_Detonator Proj1;
	local AK91_Detonator Proj2;

	if (triggerType == 1)
	{
		Proj1 = Spawn (class'AK91_Detonator',InstigatedBy,,Other.Location, Other.Rotation);
		if (Proj1 != None)
		{
			Proj1.Instigator = InstigatedBy;
			Proj1.ChargeControl = self;
