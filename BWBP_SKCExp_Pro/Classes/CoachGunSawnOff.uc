...
//=============================================================================
class CoachGunSawnOff extends BallisticProShotgun;
...
simulated event PostNetBeginPlay()
{
	...
	{
		CoachGunSawnOffPrimaryFire(FireMode[0]).bFireOnRelease = false;
	}
}
...
simulated function AnimEnded (int Channel, name anim, float frame, float rate)
{
    ...
    (
        ...
        anim == CoachGunSawnOffPrimaryFire(FireMode[0]).AimedFireEmptyAnim || 
        anim == CoachGunSawnOffPrimaryFire(FireMode[0]).AimedFireSingleAnim
    )
	{
		AnimBlendParams(1, 0);
...
function byte BestMode()
{
	...
	if (Dist > 1024)
	{
		...
		{
			CurrentWeaponMode = 1;
			CoachGunSawnOffPrimaryFire(FireMode[0]).SwitchWeaponMode(CurrentWeaponMode);
		}
	}
...
simulated function float ChargeBar()
{
	if (FireMode[0].bIsFiring)
		return FMin(1, FireMode[0].HoldTime / CoachGunSawnOffPrimaryFire(FireMode[0]).ChargeTime);
	return FMin(1, CoachGunSawnOffPrimaryFire(FireMode[0]).DecayCharge / CoachGunSawnOffPrimaryFire(FireMode[0]).ChargeTime);
}
...
defaultproperties
{
     ...
     MeleeFireClass=Class'BWBP_SKCExp_Pro.CoachGunSawnOffMeleeFire'
	 ...
	 ParamsClasses(1)=Class'CoachGunSawnOffWeaponParamsClassic'
     FireModeClass(0)=Class'BWBP_SKCExp_Pro.CoachGunSawnOffPrimaryFire'
     ...
     InventoryGroup=2
     PickupClass=Class'BWBP_SKCExp_Pro.CoachGunSawnOffPickup'
     PlayerViewOffset=(X=-10.000000,Y=20.000000,Z=-30.000000)
     AttachmentClass=Class'BWBP_SKCExp_Pro.CoachGunSawnOffAttachment'
