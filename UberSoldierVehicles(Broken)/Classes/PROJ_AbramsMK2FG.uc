class PROJ_AbramsMK2FG extends DKProjectile;
...
simulated event HitWall(vector HitNormal, actor Wall)
{
	if ( Level.NetMode != NM_DedicatedServer)
	{
        Spawn(class'PROJ_AbramsMK2FG_PS',self,,Location); 
        }
...
simulated event PhysicsVolumeChange(PhysicsVolume NewVolume)
{
	...
	{
   	         Destroy();
        Spawn(class'PROJ_AbramsMK2FG_WT',self,,Location); 
	}
}
...
defaultproperties
{
     ...
     ImpactManager=Class'UberSoldierVehicles.IM_Ammo85FG'
     SplashManager=Class'UberSoldierVehicles.IM_DK_ProjWater'
     ShakeRadius=850.000000
     MotionBlurRadius=850.000000
