class IspolinT extends DKWeapons;

var Deco_IspolinTKorpus Korpus;

var Skel_IspolinT_FlapR FlapR;
var Skel_IspolinT_FlapL FlapL;

var FX_IspolinLightEffect Light1;
var FX_IspolinLightEffect Light2;

var FX_IspolinLaserEffect LaserR;
var FX_IspolinLaserEffect LaserL;
...
function PostBeginPlay()
{
	super.PostBeginPlay();
         ...
         Korpus = Spawn(class'Deco_IspolinTKorpus',self,, Location + (vect(0,0,0) << Rotation)); 
       AttachToBone(Korpus,'A');

         FlapR = Spawn(class'Skel_IspolinT_FlapR',self,, Location + (vect(0,0,0) << Rotation)); 
       AttachToBone(FlapR,'FlapR');

         FlapL = Spawn(class'Skel_IspolinT_FlapL',self,, Location + (vect(0,0,0) << Rotation)); 
       ...
       AttachToBone(Light2,'LightL');
         ...
         LaserR = Spawn(class'FX_IspolinLaserEffect',self,,Location); 
       AttachToBone(LaserR,'LaserR');

         LaserL = Spawn(class'FX_IspolinLaserEffect',self,,Location); 
       AttachToBone(LaserL,'LaserL');
}
...
defaultproperties
{
     ...
     FireSoundClass=SoundGroup'DKoppIISound.76mm'
     AltFireSoundClass=SoundGroup'DKoppIISound.76mm'
     ProjectileClass=Class'UberSoldierVehicles.PROJ_IspolinFG'
     AltFireProjectileClass=Class'UberSoldierVehicles.PROJ_IspolinAP'
     Mesh=SkeletalMesh'DKVehiclesAnim.T18T'
