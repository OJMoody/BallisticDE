...
var CF_PickupMeshCache.PickupMeshInfo pmi;
...
{
 	if(Instigator != none && CFBallisticPawn(Instigator) != none)
		CFBallisticPawn(Instigator).EquipPickUp(InventoryType);
...
simulated function SpawnMesh()
{
	...
	pmesh = Spawn(class'CFMutator.PickupMeshActor',,,pick.Location,pick.Rotation);
	if(pmesh != none)
	{
...
simulated function Notify_HandOpen()
{
	...
	if(pmesh != none)
	{
		...
		for(i = 0; i < pmi.AlphaTex.Length; i++)
			pmesh.Skins[pmi.AlphaTex[i]] = Texture'CFMutatorTex.Controls.InvisibleTex';
	}

	Instigator.bSpecialCalcView = false;
	Instigator.bMovable = true;
	CFBallisticPawn(Instigator).bLockedView = false;
