...
simulated function Tick(float DT)
{
	...
	{
		if ( (Instigator.PlayerReplicationInfo.Team != None) && (Instigator.PlayerReplicationInfo.Team.TeamIndex == 0) || bRedTeam )
			Laser = Spawn(class'BWBPArchivePackDE.LaserActor_HMCRed',,,Location);
		else
			Laser = Spawn(class'BWBPArchivePackDE.LaserActor_HMC',,,Location);
	}
	if ( (Instigator.PlayerReplicationInfo.Team != None) && (Instigator.PlayerReplicationInfo.Team.TeamIndex == 0) || bRedTeam )
...
simulated function PostNetBeginPlay()
{
	...
	if ( (Instigator != None) && (Instigator.PlayerReplicationInfo != None) && (Instigator.PlayerReplicationInfo.Team != None) || bRedTeam )
	{
		if ( Instigator.PlayerReplicationInfo.Team.TeamIndex == 0 || bRedTeam )
			...
			Skins[0] = Shader'BallisticRecolors4.BeamCannon.BeamCannonSkin_SD';
	}
...
defaultproperties
{
     ...
     AltMuzzleFlashClass=Class'BWBPArchivePackDE.HMCFlashEmitter'
     ImpactManager=Class'BWBPArchivePackDE.IM_HMCBlast'
     InstantMode=MU_Both
     FlashMode=MU_Both
     LightMode=MU_Both
     TracerClass=Class'BWBPArchivePackDE.TraceEmitter_HMC'
