...
//=============================================================================
class FP9BombClassic extends BallisticKGrenade;
...
var   FP9TriggerClassic		MyUseTrigger;	// The trigger that sends use events
...
function InitBomb(bool bThrown, bool bLaserOn, float HoldTime)
{
	...
	{
		MyUseTrigger = Spawn(class'FP9TriggerClassic',self ,, Location);
...
function TryUse(Pawn User)
{
	...
	local FP9ExplosiveClassic Weap;
	local int AmmoAmount;
		...
		Weap = FP9ExplosiveClassic(User.FindInventoryType(class'FP9ExplosiveClassic'));
		if (Weap == None)
		{
			Weap = Spawn(class'FP9ExplosiveClassic', User,,User.Location);
			if (Weap != None)
			{
...
defaultproperties
{
     RedLaserClass=Class'BallisticDE.LaserActor_TripMineRed'
     BlueLaserClass=Class'BallisticDE.LaserActor_TripMine'
     ...
     bNoInitialSpin=True
     ImpactDamage=15
     ImpactDamageType=Class'BallisticDE.DTFP9Bomb'
     ImpactManager=Class'BallisticDE.IM_RPG'
     bRandomStartRotaion=False
     StartDelay=0.300000
     MyRadiusDamageType=Class'BallisticDE.DTFP9BombRadius'
