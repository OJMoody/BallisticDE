//=============================================================================
// DTR9Rifle.
//
// Damage type for the R9 Rifle
//
// by Nolan "Dark Carnivour" Richert.
// Copyright(c) 2005 RuneStorm. All Rights Reserved.
//=============================================================================
class DTR9Rifle extends DT_BWBullet;

defaultproperties
{
     DeathStrings(0)="%k exorcised %o with R9 rounds."
     DeathStrings(1)="%k cast the devil out of %o with %kh R9."
     DeathStrings(2)="%o was brought down by %k's ranger rifle."
     DeathStrings(3)="%o was hunted down and executed by %k."
     DeathStrings(4)="%k banged a round through %o with %kh R9."
     DeathStrings(5)="%k stapled %o with %kh R9."
     DamageIdent="Sniper"
     WeaponClass=Class'BWBPArchivePackDE.R9RangerRifle'
     DeathString="%k exorcised %o with R9 rounds."
     FemaleSuicide="%o exorcised herself."
     MaleSuicide="%o exorcised himself."
}
