...
//=============================================================================
class NTOVSecondaryFire extends BallisticMeleeFire;
...
function NotifiedDoFireEffect()
{
	...
	{
		GiveHealth(SwipeHits[i].Victim, SwipeHits[i].HitLoc, StartTrace, SwipeHits[i].HitDir, 0, 0);
		log(SWipeHits[i].Victim);
	}
	SwipeHits.Length = 0;
}

//for awarding health to allies
function GiveHealth (Actor Other, vector HitLocation, vector TraceStart, vector Dir, int PenetrateCount, int WallCount, optional vector WaterHitLocation)
{
	//local float				Dmg;
	...
	log("At start of GiveHealth");
	Victim = xPawn(Other);
	log("Victim is: "$Victim);
	if (Victim == None)
	{
		log("Give health to self");
		Instigator.GiveHealth(NTOVBandage(BW).HealAmount,Instigator.SuperHealthMax);
	}
	else
	{
		log("Victim health before: "$Victim.Health);
		Victim.GiveHealth(NTOVBandage(BW).HealAmount,Victim.SuperHealthMax);
		log("Victim health after: "$Victim.Health);
	}
}
...
defaultproperties
{
     ...
     FireAnim="UseOnSelf"
     FireRate=2.850000
     AmmoClass=Class'BWBPArchivePackDE.Ammo_NTov'
     AmmoPerFire=0
