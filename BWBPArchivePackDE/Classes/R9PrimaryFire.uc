...
state Freeze
{
	...
	{	
		...
		if (Pawn(Victim) != None && Pawn(Victim).Health > 0 && Vehicle(Victim) == None)
		{
			...
			if (Slow == None)
			{
				Pawn(Victim).CreateInventory("BWBPArchivePackDE.Inv_Slowdown");
				Slow = Inv_Slowdown(Pawn(Victim).FindInventoryType(class'Inv_Slowdown'));
			}
...
defaultproperties
{
     ...
     DamageType=Class'BWBPArchivePackDE.DTR9Rifle'
     DamageTypeHead=Class'BWBPArchivePackDE.DTR9RifleHead'
     DamageTypeArm=Class'BWBPArchivePackDE.DTR9Rifle'
     KickForce=12000
     PenetrateForce=150
     bPenetrate=True
     ...
     MuzzleFlashClass=Class'BWBPArchivePackDE.R9FlashEmitter'
     ...
     FireEndAnim=
     FireRate=0.225000
     AmmoClass=Class'BallisticDE.Ammo_348Rifle'
